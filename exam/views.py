...
def admin_teacher_view(request):
    total_salary = TMODEL.Teacher.objects.filter(status=True).aggregate(Sum('salary'))['salary__sum']
...
def admin_view_question_view(request):
    courses= models.Course.objects.all()
    return render(request, 'exam/admin_view_question.html',{'context':courses})
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            send_mail(str(name)+' || '+str(email), message,settings.EMAIL_HOST_USER, settings.EMAIL_RECEIVER_USER, fail_silently= False)
